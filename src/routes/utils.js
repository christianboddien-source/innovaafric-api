...
router.get('/rates', async (req, res) => {
  ...
  return success(res, { rates, updated_at: new Date().toISOString(), source: 'BCEAO + mercado' });
});
...
router.get('/webhooks/events', requireAuth, async (req, res) => {
  const events = await prisma.webhook.findMany({ orderBy: { createdAt: 'desc' }, take: 20 });
  const total = await prisma.webhook.count();
  return success(res, { events, total });
});
...
router.get('/health', (_req, res) => {
  return res.status(200).json({
    ...
    services: {
      xenderMoney: 'operational', xenderShop: 'operational',
      xenderBigShop: 'operational', xenderDelivery: 'operational'
    }
  });
});
